...
`ANALYSIS_MODE=llm`이어도 **정상 판정이면 LLM을 호출하지 않는다.** 판정과 수치는 통계가 강제하고 정상 상태의 서술은 `detection`이 만들어 두므로, LLM은 이상이 확정된 실행에서만(요약 → 검증 패널 → 진단) 붙는다. 5분 주기로 돌려도 정상인 날의 비용은 0이다. 예전처럼 정상에도 요약을 생성하려면 `LLM_SUMMARY_ON_NORMAL=true`.

그래도 비용을 더 제한해야 하면 `RUN_TIMES=08:00,18:00`처럼 고정 시각 모드를 쓸 수 있다. 이 모드에서는 다음 두 가지가 자동으로 달라진다.
