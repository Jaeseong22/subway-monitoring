import { Station, ArrivalInfo, Anomaly, AIInsight } from '../types';
...
// Recent timestamp for mock data
const now = new Date();
const pad = (n: number) => String(n).padStart(2, '0');
const mockRecptnDt = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;

export const mockArrivals: ArrivalInfo[] = [
// 서울역 (133) - 하행 (1)
{
  subwayId: '1001',
  updnLine: '1',
  trainLineNm: '인천행 - 인천방면',
  statnFid: '1001000132',
  statnTid: '1001000134',
  statnId: '133',
  statnNm: '서울역',
...
// 종로3가 (130) - 하행 (1)
{
  ...
  trnsitCo: '2',
  ordkey: '01001인천0',
  subwayList: '1001,1003,1005',
  statnList: '130,329,533',
  btrainSttus: '',
  barvlDt: '180',
  btrainNo: 'K201',
  bstatnId: '1001000161',
...
// 청량리 (124) - 상행 (0)
{
  ...
  trnsitCo: '3',
  ordkey: '02001소요산0',
  subwayList: '1001,1061,1063,1077',
  statnList: '124',
  btrainSttus: '',
  barvlDt: '150',
  btrainNo: 'K301',
...
export const mockAnomalies: Anomaly[] = [
{
  ...
  impactScope: '실시간 도착 정보 API (전체 역)',
  severity: '위험',
  description:
  '외부 철도청 API 연동 구간에서 응답 지연이 발생하여 캐시 데이터가 만료되고 있습니다.',
  reasoning:
  '평균 응답 시간 200ms에서 3500ms로 17배 증가. P99 지연율 95% 초과.',
  recommendedActions: [
  '외부 API 상태 확인',
  '캐시 TTL 임시 연장',
  '서킷 브레이커 수동 활성화 검토'],
  ...
  metrics: [
  { time: '10:00', value: 210, baseline: 200 },
  { time: '10:05', value: 250, baseline: 200 },
  { time: '10:10', value: 800, baseline: 200 },
  { time: '10:15', value: 3500, baseline: 200 },
  { time: '10:20', value: 3800, baseline: 200 }]
...
},
{
  ...
  impactScope: '메인 페이지 및 검색 API',
  severity: '주의',
  description:
  '특정 시간대(출근 시간) 예상치를 뛰어넘는 트래픽이 유입되고 있습니다.',
  reasoning:
  '초당 요청 수(RPS)가 평소 대비 300% 증가. 현재 서버 CPU 사용률 75% 도달.',
  recommendedActions: [
  'Auto-scaling 그룹 모니터링',
  '비핵심 백그라운드 작업 일시 중지'],

  metrics: [
  { time: '09:30', value: 500, baseline: 450 },
  { time: '09:35', value: 800, baseline: 480 },
  { time: '09:40', value: 1500, baseline: 500 },
  { time: '09:45', value: 1800, baseline: 500 },
  { time: '09:50', value: 1750, baseline: 480 }]
...
},
{
  ...
  impactScope: '역 상세 정보 조회 API',
  severity: '주의',
  description:
  '특정 역(신도림, 구로) 조회 시 500 Internal Server Error 발생 빈도가 증가했습니다.',
  reasoning: '최근 10분간 5xx 에러율 2.5% 도달 (임계치 1% 초과).',
  recommendedActions: [
  'DB 커넥션 풀 상태 확인',
  '해당 역 데이터 무결성 검증'],

  metrics: [
  { time: '08:00', value: 0.1, baseline: 0.1 },
  { time: '08:10', value: 0.2, baseline: 0.1 },
  { time: '08:20', value: 1.5, baseline: 0.1 },
  { time: '08:30', value: 2.5, baseline: 0.1 },
  { time: '08:40', value: 2.2, baseline: 0.1 }]
...
export const mockInsights: AIInsight[] = [
{
  id: 'i1',
  summary:
  '평소 대비 실시간 도착 API 응답 지연이 17배 증가했습니다. 외부 연동 구간 점검이 시급합니다.',
  type: 'performance'
},
{
  id: 'i2',
  summary:
  '출근 시간대 트래픽이 예측치보다 300% 높게 발생 중이나, Auto-scaling이 정상 작동하여 서비스 영향은 없습니다.',
  type: 'traffic'
},
{
  id: 'i3',
  summary:
  '신도림, 구로역 등 환승역 위주로 데이터베이스 조회 에러가 간헐적으로 발생하고 있습니다.',
  type: 'error'
}];